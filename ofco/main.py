...
def motion_compensate(stack1, stack2, output1, output2, frames, param, verbose=False, parallel=True):     
    start = timer()
    stack1 = stack1[frames,:,:].astype(np.float64)
    stack1_rescale = (stack1 - np.amin(stack1)) / (np.amax(stack1) - np.amin(stack1)) * 255
    ...
    # Motion estimation
    start = timer()
    w = np.zeros([stack1_rescale.shape[0] - 1, stack1_rescale.shape[1], stack1_rescale.shape[2], 2])
    ...
    for t in range(len(frames) - 1):
        stack1_warped[t + 1, :, :] = bilinear_interpolate(stack1[t + 1, :, :], w[t, :, :, 0], w[t, :, :, 1])
        stack2_warped[t + 1, :, :] = bilinear_interpolate(stack2[t + 1, :, :], w[t, :, :, 0], w[t, :, :, 1])
    end = timer()
    if verbose:
...
def main(stack1, stack2, output_dir, frames=-1, verbose=False, **kwargs):
    ...
    motion_compensate(stack1, stack2, output1, output2, frames, param, verbose, parallel=False)

if __name__ == "__main__":
    main('/mnt/internal_hdd/aymanns/181220_Rpr_R57C10_GC6s_tdTom/Fly5/002_coronal/2p/red.tif',
         '/mnt/internal_hdd/aymanns/181220_Rpr_R57C10_GC6s_tdTom/Fly5/002_coronal/2p/green.tif',
         'results/test_181220_Fly5_002_module', verbose=True, frames=2)
