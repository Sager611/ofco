...
def partial_deriv(imgs, w, deriv_filter=np.array([[1, -8, 0, 8, -1]])/12):
    ...
    I1y = correlate2d(imgs[:, :, 0], deriv_filter.transpose(), boundary='symm', mode='same')
    b = 0.5
    Ix = b * Ix + (1 - b) * I1x
    Iy = b * Iy + (1 - b) * I1y
    Ix[indx] = 0
...
def denoise_color_weighted_medfilt2d(uv, im, occ=None, bfhsz=None, mfsz=None, sigma_i=5, sigma_x=20, fullVersion=False):
    """
    ...
    for ib in range(1, nB + 1):
        ...
        pad_indx = np.tile(nindx.transpose().flatten().reshape(-1, 1), [1, len(indx_row)]) +\
        ...
        for i in range(pad_im.shape[2]):
            ...
            tmp_w = tmp_w + (tmp[pad_indx] - np.tile(tmp[flat_cindx].transpose(), [(bfhsz * 2 + 1) ** 2, 1])) ** 2
...
def post_process(w, I1, I2, sigmaS, sigmaC):
    ...
    wOut = denoise_color_weighted_medfilt2d(w, stack, medfilt2d(occ, [9, 9]), int(np.ceil(sigmaS)), 3, int(np.ceil(sigmaC)), int(np.ceil(sigmaS)), True)
    return wOut


def default_parameters():
    #lmbd stands for lambda which is a build in in python
    param = {'lmbd':1000, 'gamma':100, 'mu':0.1, 'nu':0.1, 'c2fSpacing':1.5, 'threshMatch':70, 'occThresh':0.5,'consistencyTol':2,'changeTol':1e-3, 'nbWarps':1, 'sigmaS':1, 'sigmaC':200, 'maxIters':150, 'iWM':50}
    return param
...
def crop_fit_size_center(f, target_size):
    ...
    f_out = f[shift[0]:shift[0] + target_size[0], shift[1]:shift[1] + target_size[1], shift[2]:shift[2] + target_size[2]]
    return f_out
